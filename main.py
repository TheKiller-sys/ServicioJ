...
def enviar_reporte_ventas():
    """Envia un reporte de ventas al usuario cada 5 minutos."""
    for user_id, user_data in usuarios_activos.items():
        vendedor_id = user_data["id"]
        ventas = db.obtener_ventas_diarias(vendedor_id)
        cantidad_ventas = len(ventas)
        total_ganancias = sum(venta[1] * venta[2] for venta in ventas) #calcula las ganancias
        nombre_vendedor = db.obtener_vendedor(user_data["usuario"])[2] #obtener nombre del vendedor

        mensaje = f"Hola {nombre_vendedor},\nReporte de ventas hasta el momento:\nHas realizado {cantidad_ventas} ventas.\nGanancia total: ${total_ganancias:.2f}"
        try:
            bot.send_message(user_id, mensaje)
            logging.info(f"Reporte de ventas enviado al usuario {user_id}.")
        except Exception as e:
            logging.error(f"Error al enviar reporte de ventas al usuario {user_id}: {e}")
...
schedule.every(5).minutes.do(enviar_reporte_ventas) # Agrega la tarea programada para el reporte de ventas
