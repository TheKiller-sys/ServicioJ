...
def verificar_contrasena(message, contrasena_correcta):
    ...
    if contrasena == contrasena_correcta:
        ...
        # Iniciar el envío de mensajes cada 5 minutos para este usuario
        programar_envio_estadisticas(message.chat.id)
    else:
...
# --- Envío de Estadísticas Programado ---
def enviar_estadisticas(chat_id):
    """Envía la cantidad de productos vendidos en el día al usuario."""
    if chat_id in usuarios_activos:
        vendedor_id = usuarios_activos[chat_id]["id"]
        ventas = db.obtener_ventas_diarias(vendedor_id)
        total_productos_vendidos = sum(venta[1] for venta in ventas)
        nombre_vendedor = db.obtener_vendedor(usuarios_activos[chat_id]["usuario"])[2]

        mensaje = f"Hola {nombre_vendedor}, hasta el momento has vendido {total_productos_vendidos} productos hoy."
        try:
            bot.send_message(chat_id, mensaje)
            logging.info(f"Estadísticas diarias enviadas al usuario {chat_id}.")
            enviar_log(f"Estadísticas diarias enviadas al usuario {chat_id}.", "INFO")
        except Exception as e:
            logging.error(f"Error al enviar estadísticas diarias al usuario {chat_id}: {e}")
            enviar_log(f"Error al enviar estadísticas diarias al usuario {chat_id}: {e}", "ERROR")
    else:
        logging.warning(f"Intento de enviar estadísticas a un usuario no autenticado (Chat ID: {chat_id}).")
        enviar_log(f"Intento de enviar estadísticas a un usuario no autenticado (Chat ID: {chat_id}).", "WARNING")


def programar_envio_estadisticas(chat_id):
    """Programa el envío de estadísticas cada 5 minutos para un usuario específico."""
    schedule.every(5).minutes.do(enviar_estadisticas, chat_id=chat_id)
